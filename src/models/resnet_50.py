...
def get_model(input_shape=(300, 80, 1), embeddings_size=64):
    ...
    # Stage 2
    X = blocks.convolutional_block(X, f=3, filters=[64, 64, 256], stage=2, block='a', s=1)
    X = blocks.identity_block(X, 3, [64, 64, 256], stage=2, block='b')
    X = blocks.identity_block(X, 3, [64, 64, 256], stage=2, block='c')

    # Stage 3 (≈4 lines)
    X = blocks.convolutional_block(X, f=3, filters=[128, 128, 512], stage=3, block='a', s=2)
    ...
    # Stage 4 (≈6 lines)
    X = blocks.convolutional_block(X, f=3, filters=[256, 256, 1024], stage=4, block='a', s=2)
    ...
    # Stage 5 (≈3 lines)
    X = blocks.convolutional_block(X, f=3, filters=[512, 512, 2048], stage=5, block='a', s=2)
